...
class Deliveryboy extends REST_Controller {
    ...
    function pick_order_post(){
        ...
        $this->common_model->data_insert("order_status",array("order_id"=>$order->order_id,"status"=>ORDER_OUT_OF_DELIVEY),true);
    ...
    function delivered_order_post(){
        ...
        if(!empty($order)){
            ...
            $message = _l("Thanks being with Seyyar, Your Order No. #order_no# is delivered, Please share your reviews.");
            $message = str_replace("#order_no#",$order->order_no,$message);
        }
        if($title != "" && $message != ""){
            $this->load->library("onesignallib");
        ...
        $this->common_model->data_insert("order_status",array("order_id"=>$order->order_id,"status"=>ORDER_DELIVERED),true);
