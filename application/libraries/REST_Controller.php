...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        date_default_timezone_set(get_option("date_time_zone","GMT"));
    }
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        if(isset($request_header[X_APP_DEVICE])){
            $device = $request_header[X_APP_DEVICE];
        }
        if(isset($request_header[X_APP_LANGUAGE])){
            $language = $request_header[X_APP_LANGUAGE];
        }
        if(isset($request_header[X_APP_VERSION])){
            $version = $request_header[X_APP_VERSION];
        }
